...
export function HireTerminationSummaryCard({
  ...
  return (
    <section className="rph-card space-y-3 p-4">
      <div>
        <h2 className="text-sm font-semibold text-rph-fg">Contract termination summary</h2>
        <p className="rph-muted mt-1 text-xs">
          Snapshot recorded when the contract was ended.
        </p>
      </div>

      <dl className="grid gap-3 text-sm sm:grid-cols-2">
        <div>
          <dt className="text-xs text-rph-fg-muted">Accrued rent due</dt>
          <dd className="mt-0.5 font-medium tabular-nums text-rph-fg">
            {formatGbp(summary.accruedRentDueGbp)}
          </dd>
        </div>
        <div>
          <dt className="text-xs text-rph-fg-muted">Accrued rent paid</dt>
          <dd className="mt-0.5 font-medium tabular-nums text-rph-fg">
            {formatGbp(summary.accruedRentPaidGbp)}
          </dd>
        </div>
        <div>
          <dt className="text-xs text-rph-fg-muted">Prepaid rent credit</dt>
          <dd className="mt-0.5 font-medium tabular-nums text-rph-fg">
        ...
        <div className="sm:col-span-2">
          <dt className="text-xs text-rph-fg-muted">Net settlement at termination</dt>
          <dd className="mt-0.5 font-semibold tabular-nums text-rph-fg">
            {settlementBalanceLabel(summary.balanceDirection, Math.abs(summary.netSettlementGbp))}
          </dd>
        </div>
        {depositDispositionLabel ? (
          <div>
            <dt className="text-xs text-rph-fg-muted">Deposit disposition</dt>
            <dd className="mt-0.5 text-rph-fg">{depositDispositionLabel}</dd>
          </div>
        ) : null}
        {settlementResolutionLabel ? (
          <div>
            <dt className="text-xs text-rph-fg-muted">Settlement resolution</dt>
