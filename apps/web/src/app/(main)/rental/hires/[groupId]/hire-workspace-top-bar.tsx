  ...
  return (
    <div className="-mx-3 -mt-3 mb-5 px-3 pt-3">
      <div className="hire-ws-top-back-row">
        <Link href="/rental/hires" className="hire-ws-back-link">
          <span aria-hidden>←</span> Back to hires
        </Link>
        ...
        {hires.length > 1 ? (
          <div className="relative min-w-0" ref={rootRef}>
            <button
              type="button"
              className="hire-ws-hire-switcher"
              aria-expanded={open}
              aria-label="Switch hire"
              onClick={() => setOpen((v) => !v)}
            >
              <span className="truncate font-mono">{shell.vehicleVrm}</span>
              <span className="shrink-0 text-[10px] text-rph-fg-muted" aria-hidden>
                ▾
              </span>
            </button>

            {open ? (
              <div className="absolute left-0 z-40 mt-1 w-[min(100vw-2rem,22rem)] overflow-hidden rounded-lg border border-rph-border bg-rph-elevated shadow-lg sm:left-auto sm:right-0">
