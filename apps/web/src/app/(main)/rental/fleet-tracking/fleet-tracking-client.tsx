...
export function FleetTrackingClient({ canManage }: { canManage: boolean }) {
  ...
  const [apiDebugJson, setApiDebugJson] = useState<string | null>(null);
  ...
  function saveCredentials() {
    ...
    startTransition(async () => {
      ...
      if (res.apiDebug) {
        const json = JSON.stringify(res.apiDebug, null, 2);
        setApiDebugJson(json);
        console.info("[fleet-tracking] authorization API debug", res.apiDebug);
      }
      if (!res.ok) {
        setOverlay({
          phase: "error",
    ...
    <div className="space-y-8">
      <div>
        ...
        <p className="rph-muted mt-1 text-sm">
          Connect SmartCar Tracker, map devices to your fleet VRMs, and review live mileage reports.
        </p>
      </div>
      ...
      {apiDebugJson ? (
        <div className="rph-card space-y-2 p-4">
          <div className="flex flex-wrap items-center justify-between gap-2">
            <p className="text-xs font-semibold uppercase tracking-wide text-rph-fg-muted">
              SmartCar Tracker API response (share with provider)
            </p>
            <button
              type="button"
              className="rph-btn-ghost h-8 px-3 text-xs"
              onClick={() => {
                void navigator.clipboard.writeText(apiDebugJson);
                setMessage("API response copied to clipboard.");
              }}
            >
              Copy
            </button>
          </div>
          <pre className="max-h-64 overflow-auto rounded-lg bg-rph-chrome p-3 text-left font-mono text-xs text-rph-fg whitespace-pre-wrap">
            {apiDebugJson}
          </pre>
          <p className="rph-meta">
            Also logged in the server terminal as{" "}
            <span className="font-mono">[fleet-tracking] authorization response</span>. Password and signature are not
            included.
          </p>
        </div>
      ) : null}
      ...
      <section className="rph-card space-y-4 p-4 sm:p-5">
        ...
        <p className="rph-meta">
          Enter the SmartCar Tracker Open API account for this company. Passwords are encrypted at rest and never shown
          again.
        </p>
      ...
      <section className="rph-card space-y-4 p-4 sm:p-5">
        <div className="flex flex-wrap items-center justify-between gap-2">
          <div>
            <h2 className="text-xs font-semibold uppercase tracking-wide text-rph-fg-muted">Weekly mileage</h2>
            <p className="rph-meta mt-1">Last 7 days from the tracker (primary device only), shown in miles.</p>
          </div>
