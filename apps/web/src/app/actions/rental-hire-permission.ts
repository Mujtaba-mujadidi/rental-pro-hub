...
import { canManageSettings, canReadRentals } from "@/lib/auth/rental-permissions";
...
async function requireCompanyPermissionLetterAdmin() {
  const { profile } = await requireRentalCompanyArea();
  const writable = await assertRentalCompanyWritable(profile);
  if (!writable.ok) throw new Error(writable.error);
  if (!canManageSettings(profile)) throw new Error("You do not have permission to manage the permission letter.");
...
export async function listCompanyHirePermissionLetterVersionsAction(): Promise<
...
export async function saveCompanyHirePermissionLetterDraftAction(
  formData: FormData,
): Promise<{ ok: true; id: string } | { ok: false; error: string }> {
  try {
    const { companyId } = await requireCompanyPermissionLetterAdmin();
...
export async function publishCompanyHirePermissionLetterVersionAction(
  versionId: string,
): Promise<{ ok: true } | { ok: false; error: string }> {
  try {
    const { profile, companyId } = await requireCompanyPermissionLetterAdmin();
...
export async function republishArchivedCompanyHirePermissionLetterVersionAction(
  versionId: string,
): Promise<{ ok: true } | { ok: false; error: string }> {
  try {
    const { profile, companyId } = await requireCompanyPermissionLetterAdmin();
...
export async function archiveCompanyHirePermissionLetterVersionAction(
  versionId: string,
): Promise<{ ok: true } | { ok: false; error: string }> {
  try {
    await requireCompanyPermissionLetterAdmin();
    const trimmed = versionId?.trim();
    if (!trimmed) return { ok: false, error: "Missing version id." };
...
export async function loadPublishedHirePermissionLetterForWizardAction(): Promise<
  | { ok: true; title: string; body: string; versionLabel: string }
...
> {
  try {
    ...
    const { data, error } = await supabase
      ...
      .maybeSingle();
    if (error) return { ok: false, error: error.message };
    if (!data || !String(data.body ?? "").trim()) {
      return { ok: false, error: "Publish a permission letter in Settings before creating hire contracts." };
    }
    return {
      ok: true,
      title: (data.title as string) || "Permission letter",
      body: data.body as string,
      versionLabel: (data.version_label as string) || "Published",
    };
  } catch (e) {
    return { ok: false, error: e instanceof Error ? e.message : "Could not load permission letter." };
  }
}
