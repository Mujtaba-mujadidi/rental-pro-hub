-- Company invites must never become drivers.
-- If app_role=rental_company but company_id is missing/invalid, still create a rental profile
-- (app ensureRentalCompanyMembership attaches the company). Never fall through to role=driver.
...
begin
  ...
  if v_company_id is not null then
    select exists(select 1 from public.companies c where c.id = v_company_id) into v_company_ok;
  end if;
  ...
  -- Rental invite / company contact: never create a driver profile.
  if v_app_role = 'rental_company' then
    if v_company_ok then
      ...
      -- Metadata says rental but company missing/invalid — still not a driver.
  ...
  -- Only create driver_profiles for explicit driver signup (not bare invites / dashboard users).
