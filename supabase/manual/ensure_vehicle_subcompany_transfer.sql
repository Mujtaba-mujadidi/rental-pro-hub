-- Vehicle subcompany transfer (idempotent). Mirrors 20260809180000_vehicle_subcompany_transfer.sql.

alter table public.vehicle_documents
  add column if not exists version_status text not null default 'current';

alter table public.vehicle_documents
  drop constraint if exists vehicle_documents_version_status_check;
alter table public.vehicle_documents
  add constraint vehicle_documents_version_status_check
...
alter table public.vehicle_hire_inspections
  drop constraint if exists vehicle_hire_inspections_completion_mode_check;
alter table public.vehicle_hire_inspections
  add constraint vehicle_hire_inspections_completion_mode_check
...
alter table public.vehicle_subcompany_transfer_intents
  drop constraint if exists vehicle_subcompany_transfer_intents_status_check;
alter table public.vehicle_subcompany_transfer_intents
  add constraint vehicle_subcompany_transfer_intents_status_check
...
begin
  if not exists (
    select 1 from pg_constraint where conname = 'vehicle_transfers_transfer_intent_id_fkey'
  ) then
    alter table public.vehicle_transfers
...
begin
  if not exists (
    select 1 from pg_constraint where conname = 'vehicle_hire_inspections_transfer_intent_id_fkey'
  ) then
    alter table public.vehicle_hire_inspections
...
begin
  if not exists (
    select 1 from pg_constraint where conname = 'vehicle_documents_vehicle_transfer_id_fkey'
  ) then
    alter table public.vehicle_documents
...
alter table public.vehicle_transfer_document_requirements
  drop constraint if exists vehicle_transfer_document_requirements_document_kind_check;
alter table public.vehicle_transfer_document_requirements
  add constraint vehicle_transfer_document_requirements_document_kind_check
  check (
    document_kind in (
      'logbook',
...
alter table public.vehicle_transfer_document_requirements
  drop constraint if exists vehicle_transfer_document_requirements_status_check;
alter table public.vehicle_transfer_document_requirements
  add constraint vehicle_transfer_document_requirements_status_check
