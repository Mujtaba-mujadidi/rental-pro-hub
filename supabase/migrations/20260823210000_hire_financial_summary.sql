-- Hire financial summary (read model) + durable extra-charge payment allocations.
-- Source of truth remains schedule / balance payments / charge line items.
-- Summaries are rebuilt after payment and charge mutations via HireFinanceService.
...
-- Durable per-payment allocations onto charge lines (and later schedule rows).
create table if not exists public.vehicle_hire_payment_allocations (
  ...
  target_type text not null check (target_type in ('driver_charge_line', 'schedule_row')),
  target_id uuid not null,
  amount_gbp numeric(12, 2) not null check (amount_gbp > 0),
  created_at timestamptz not null default now(),
  unique (balance_payment_id, target_type, target_id)
);
...
-- Denormalised paid / status on charge lines for Payments UI (rebuilt with summary).
